...
import torch
from transformers import BitsAndBytesConfig, Qwen2VLForConditionalGeneration, AutoProcessor, AutoConfig, AutoTokenizer
import warnings
import os
import tensorboard
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, 
    ...
    if 'lora' in model_name.lower() and model_base is not None:
        ...
        # target_embedding_size = non_lora_trainables['model.embed_tokens.weight'].size(0)
        # original_embedding_size = model.state_dict()['model.embed_tokens.weight'].size(0)
        # # print(original_embedding_size) # 151936

        # if original_embedding_size > target_embedding_size:
        #     model.state_dict()['model.embed_tokens.weight'] = model.state_dict()['model.embed_tokens.weight'][:target_embedding_size, :]
        ...
        processor = AutoProcessor.from_pretrained(model_base)
        model = Qwen2VLForConditionalGeneration.from_pretrained(model_base, low_cpu_mem_usage=True, **kwargs)
        # print("Vocabulary size:", model.get_input_embeddings().weight.shape[0])
        # processor.tokenizer.add_tokens(["[RET]"])
        # model.resize_token_embeddings(len(processor.tokenizer))
        # print("Vocabulary size:", model.get_input_embeddings().weight.shape[0])
